app.controller('productsCtrl', function ($scope, $modal, $filter, Data) {
    $scope.product = {};
    Data.get('producto/all').then(function(data){
    ...
    $scope.open = function (p,size) {
        var modalInstance = $modal.open({
          templateUrl: '../parciales/productos/productEdit.html',
...
app.controller('productEditCtrl', function ($scope, $modalInstance, item, Data) {
  ...
  $scope.product = angular.copy(item);
        ...
        $scope.saveProduct = function (product) {
            ...
            if(product.id > 0){
                Data.put('producto/update/'+product.id, product).then(function (result) {
                ...
                Data.post('producto/insert', product).then(function (result) {
                    if(result.status != 'error'){
                        var x = angular.copy(product);
                        x.save = 'insert';
                        x.id = result.data;
